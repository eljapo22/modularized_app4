...
def migrate_data(conn):
    ...
    # First, let's verify the structure of existing tables
    print("Verifying existing tables...")
    tables = conn.execute("SHOW TABLES").fetchdf()
    print("Available tables:", tables['name'].tolist())
    
    # Get schema of transformer_loading
    print("\nTransformer Loading schema:")
    schema = conn.execute("DESCRIBE transformer_loading").fetchdf()
    print(schema)
    
    # Migrate transformer base data with simplified query
    print("\nMigrating transformer base data...")
    conn.execute("""
    ...
    conn.execute("""
        ...
        SELECT 
            ...
            CASE 
                ...
                ELSE 'Normal'
            END as alert_level,
            AVG(loading_percentage) as avg_loading_24h,
            MAX(loading_percentage) as max_loading_24h
        FROM transformer_loading
    ...
    conn.execute("""
        ...
        SELECT DISTINCT
            customer_id,
            transformer_id,
            CURRENT_DATE as connection_date,
            'Active' as status
        FROM customer_reading
        WHERE customer_id IS NOT NULL AND transformer_id IS NOT NULL
