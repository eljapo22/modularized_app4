                ...
                if search_clicked:
                    ...
                    if not all([start_date, end_date, selected_feeder, selected_transformer]):
                        ...
                        if transformer_data is not None and not transformer_data.empty:
                            ...
                            transformer_tab, loading_tab, customer_tab = st.tabs([
                                "Transformer Data",
                                "Loading Status",
                                "Customer Data"
                            ])
                            ...
                            with transformer_tab:
                                ...
                                if transformer_data is not None and not transformer_data.empty:
                                    create_section_header("Power Consumption")
                            ...
                            with loading_tab:
                                if transformer_data is not None and not transformer_data.empty:
                                    charts.display_loading_status_tab(transformer_data)
                                else:
                                    st.warning("No loading data available")

                            with customer_tab:
                                if customer_data is not None and not customer_data.empty:
                                    display_customer_tab(customer_data)
                                else:
                                    st.info("No customer data available for the selected transformer")
                        else:
